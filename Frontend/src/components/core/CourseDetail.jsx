...
import Footer from './Footer'; // ✅ Shared footer — no duplication

// ─── Navbar (same shared nav as CatalogItem) ──────────────────────────────────
  ...
  return (
    <nav style={{
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8, cursor: "pointer" }} onClick={() => navigate('/')}>
        <div style={{ width: 28, height: 28, background: "#FFD60A", borderRadius: 6, display: "flex", alignItems: "center", justifyContent: "center" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
        ...
        <div style={{ width: 32, height: 32, borderRadius: "50%", background: "#FFD60A", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer" }}>
      ...
      {[1, 2, 3, 4, 5].map((s) => {
        ...
        return (
          <svg key={s} width={size} height={size} viewBox="0 0 20 20"
            fill={filled ? "#E7C009" : half ? "url(#half2)" : "none"}
    ...
    <div style={{ borderBottom: "1px solid #2C3244" }}>
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <span style={{ color: "#AFB2BF", fontSize: 13, transition: "transform 0.2s", display: "inline-block", transform: open ? "rotate(0deg)" : "rotate(-90deg)" }}>∧</span>
          <span style={{ color: "#F1F2FF", fontWeight: 600, fontSize: 14 }}>{section.sectionName || section.name}</span>
        </div>
        <span style={{ color: "#FFD60A", fontSize: 12, fontWeight: 500 }}>
          {section.subSection?.length || 0} lectures &nbsp; {section.totalDuration || "51min"}
        </span>
      </div>
            ...
            <div key={lecture._id || i}>
              ...
              <div style={{
                ...
                <div style={{ display: "flex", gap: 10, alignItems: "flex-start", flex: 1 }}>
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#AFB2BF" strokeWidth="2" style={{ marginTop: 2, flexShrink: 0 }}>
                  ...
                  <div>
                    ...
                    {lecture.description && (
                      <p style={{ color: "#AFB2BF", fontSize: 12, lineHeight: 1.5 }}>{lecture.description}</p>
                    )}
                  </div>
                </div>
                <span style={{ color: "#AFB2BF", fontSize: 12, flexShrink: 0, marginLeft: 16 }}>
                  {lecture.timeDuration || "02:09"}
                </span>
              </div>
            </div>
          ))}
    ...
    <div style={{
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
        {avatar
          ...
          : <div style={{ width: 38, height: 38, borderRadius: "50%", background: "#FFD60A", display: "flex", alignItems: "center", justifyContent: "center" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
        <span style={{ color: "#E7C009", fontWeight: 700, fontSize: 13 }}>{Number(rating).toFixed(1)}</span>
        <StarRating rating={parseFloat(rating)} size={13} />
      </div>
    </div>
  );
}
...
// ─── Main Component ───────────────────────────────────────────────────────────
const CourseDetail = () => {
  const { courseId } = useParams(); // ✅ expects route: /courses/:courseId
  const navigate = useNavigate();
  ...
  useEffect(() => {
    const fetchCourse = async () => {
      try {
        const response = await axios.get(`http://localhost:3000/api/v1/course/getCourseDetails/${courseId}`);
  ...
  // ── Derived values (adjust keys to match your API) ──
  ...
  const thumbnail      = course?.thumbnail || "https://images.unsplash.com/photo-1587620962725-abab7fe55159?w=600&q=80";
  const language       = course?.language || "English";
  const createdAt      = course?.createdAt ? new Date(course.createdAt).toLocaleDateString('en-US', { month: '2-digit', year: 'numeric' }) : "02/2020";
  ...
  const whatYouLearn   = course?.whatYouLearn
    ? (Array.isArray(course.whatYouLearn) ? course.whatYouLearn : course.whatYouLearn.split('\n').filter(Boolean))
    : [
        ...
        <>
            ...
            <div style={{ maxWidth: 1200, margin: "0 auto", display: "flex", gap: 40, alignItems: "flex-start" }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                <div style={{ display: "flex", gap: 8, alignItems: "center", marginBottom: 14, fontSize: 13, color: "#AFB2BF" }}>
                  ...
                  <span>/</span>
                  <span style={{ color: "#FFD60A", fontWeight: 600 }}>{course?.category?.name || "Python"}</span>
                </div>

                <h1 style={{ fontSize: 30, fontWeight: 800, color: "#F1F2FF", marginBottom: 12, lineHeight: 1.3 }}>{title}</h1>
                <p style={{ color: "#AFB2BF", fontSize: 14, lineHeight: 1.7, marginBottom: 14, maxWidth: 640 }}>{description}</p>

                {/* Rating row */}
                <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10, flexWrap: "wrap" }}>
                  <span style={{ color: "#E7C009", fontWeight: 700, fontSize: 15 }}>{avgRating}</span>
                  ...
                  <span style={{ color: "#AFB2BF", fontSize: 13 }}>{studentsCount.toLocaleString()} students</span>
                </div>

                <p style={{ color: "#AFB2BF", fontSize: 13, marginBottom: 6 }}>
                  Created by <span style={{ color: "#FFD60A", fontWeight: 600 }}>{instructorName}</span>
                </p>
                <div style={{ display: "flex", gap: 20, alignItems: "center", flexWrap: "wrap" }}>
                  <span style={{ color: "#AFB2BF", fontSize: 13, display: "flex", alignItems: "center", gap: 5 }}>
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#AFB2BF" strokeWidth="2"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>
                    Created at {createdAt}
                  </span>
                  <span style={{ color: "#AFB2BF", fontSize: 13, display: "flex", alignItems: "center", gap: 5 }}>
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#AFB2BF" strokeWidth="2"><circle cx="12" cy="12" r="10"/><line x1="2" y1="12" x2="22" y2="12"/><path d="M12 2a15.3 15.3 0 0 1 4 10 15.3 15.3 0 0 1-4 10 15.3 15.3 0 0 1-4-10 15.3 15.3 0 0 1 4-10z"/></svg>
                    {language}
                  </span>
                </div>
              </div>
              ...
              <div style={{
                ...
                <img src={thumbnail} alt={title} style={{ width: "100%", height: 180, objectFit: "cover", display: "block" }} />
                <div style={{ padding: "20px 20px 24px" }}>
                  ...
                  >Buy now</button>
                  <p style={{ color: "#AFB2BF", fontSize: 12, textAlign: "center", marginBottom: 16 }}>30-Day Money-Back Guarantee</p>

                  <p style={{ color: "#F1F2FF", fontWeight: 600, fontSize: 13, marginBottom: 10 }}>This course includes:</p>
                  {INCLUDES.map((item, i) => (
          ...
          <div style={{ maxWidth: 1200, margin: "0 auto", padding: "40px 40px" }}>
            ...
            <section style={{
              ...
              padding: "28px 28px", marginBottom: 48, background: "#161D29",
            }}>
              <h2 style={{ color: "#F1F2FF", fontSize: 20, fontWeight: 700, marginBottom: 18 }}>What you'll learn</h2>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px 32px" }}>
                {whatYouLearn.map((point, i) => (
                  <div key={i} style={{ display: "flex", gap: 10, alignItems: "flex-start" }}>
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#47A992" strokeWidth="2.5" style={{ flexShrink: 0, marginTop: 2 }}>
            ...
            <section style={{ marginBottom: 48 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                ...
                <button
                  ...
                  style={{ background: "none", border: "none", color: "#FFD60A", fontSize: 13, fontWeight: 600, cursor: "pointer" }}>
                  {allSectionsOpen ? "Collapse all sections" : "Expand all sections"}
                </button>
              </div>
              <p style={{ color: "#AFB2BF", fontSize: 13, marginBottom: 16 }}>
                {sections.length} sections • {totalLectures} lectures • {course?.totalDuration || "7h 57m"} total length
              </p>
              <div style={{ border: "1px solid #2C3244", borderRadius: 8, overflow: "hidden" }}>
                {sections.length > 0
                  ? sections.map((section, i) => (
                      <AccordionSection
                        key={section._id || i}
                        section={{ ...section, defaultOpen: allSectionsOpen || i === 0 }}
                      />
                    ))
                  : (
            ...
            <section style={{ marginBottom: 48 }}>
              ...
              <div style={{ display: "flex", gap: 16, alignItems: "flex-start" }}>
                {instructorImg
                  ? <img src={instructorImg} alt={instructorName} style={{ width: 60, height: 60, borderRadius: "50%", objectFit: "cover", flexShrink: 0 }} />
                  : <div style={{ width: 60, height: 60, borderRadius: "50%", background: "#FFD60A", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                      <span style={{ fontWeight: 700, color: "#000", fontSize: 22 }}>{instructorName.charAt(0)}</span>
                    </div>
                }
                <div>
                  <p style={{ color: "#F1F2FF", fontWeight: 700, fontSize: 16, marginBottom: 6 }}>{instructorName}</p>
                  {instructorBio && (
                    <p style={{ color: "#AFB2BF", fontSize: 13, lineHeight: 1.7, maxWidth: 680 }}>{instructorBio}</p>
                  )}
                </div>
              </div>
            </section>
