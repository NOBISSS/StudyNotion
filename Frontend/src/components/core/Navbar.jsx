...
const Navbar = () => {
  ...
  const fetchSubLinks = async () => {
    try {
      ...
      setSubinks(result.data.data.category);
    } catch (error) {
              ...
              <li key={index}>
                {link.title === "Catalog" ? (
                  <div className="relative flex items-center gap-1 justify-center group">
                    ...
                    <div
                      className="absolute top-full left-1/2 z-50 mt-2 w-[250px] -translate-x-1/2
                                        ...
                                        "
                    >
                      <div className="absolute left-[55%] top-0 h-6 w-6 rotate-45 rounded bg-white translate-y-[-10px]"></div>
                      {subLinks.length ? (
                          ...
                          >
                            <div
                              className="h-[50px] mb-4
                                                    ...
                                                    "
                            >
                              <p className="ml-2">{subLink.name}</p>
                            </div>
                          </Link>
                        ))
                      ) : (
                  ...
                  <Link to={link?.path}>
                    <p
                      className={`${matchRoute(link?.path) ? "text-[#FFD60A]" : "text-[#DBDDEA]"}`}
                    >
                      {link?.title}
                    </p>
                  </Link>
