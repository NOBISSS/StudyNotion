...
const CourseBuilderForm = ({ courseId }) => {
  ...
  // expandedSection: Set of sectionIds currently open
  const [expandedSections, setExpandedSections] = useState(new Set())

  // subsections stored as { [sectionId]: [...subsections] }
  // so each section's lectures are independent and don't bleed into each other
  ...
  const toggleSection = useCallback(async (sectionId) => {
    ...
    if (!fetchedSections.current.has(sectionId)) {
      ...
      const subs = (res?.subsections || []).map((s) => ({
        ...s,
        sectionId, // attach sectionId so we always know which section it belongs to
      }))
      setSubsectionMap((prev) => ({ ...prev, [sectionId]: subs }))
    }
  }, [expandedSections])
  ...
  const handleSectionUpdate = async (sectionId) => {
    if (!editingSection.editingName.trim()) { toast.error("Section name cannot be empty"); return }
    await updateSection({ sectionId, sectionName: editingSection.editingName.trim() })
    setSections((prev) =>
      prev.map((s) => s._id === sectionId ? { ...s, name: editingSection.editingName.trim() } : s)
    )
  ...
  // Called from EditLectureModal after a successful edit (no new video)
  ...
  // Called from EditLectureModal after a new lecture is uploaded via VideoUploaderUppy
  ...
  // ── Open edit modal — fetch subsection details ─────────────────────────────
  const openEditModal = async (lecture) => {
    // Set basic data immediately so modal opens fast
    setEditingLecture({ ...lecture, isEditing: true })
    setShowModal(true)
    // Fetch full details (video URL etc.) in background
            ...
            <div key={section._id} className="rounded-lg overflow-hidden border border-[#2C333F]">
              ...
              <div className="flex items-center justify-between px-4 py-3 bg-[#2C333F]">
                ...
                <div className="flex items-center gap-1 flex-shrink-0 ml-2">
                  {isEditingThis ? (
                    <>
                      <button onClick={() => handleSectionUpdate(section._id)}
                        className="p-1.5 text-[#838894] hover:text-green-400 transition-colors" title="Save">
                        <CheckIcon className="w-4 h-4" />
                      </button>
                      <button onClick={() => setEditingSection({ sectionId: null, isEditing: false, editingName: "" })}
                        className="p-1.5 text-[#838894] hover:text-red-400 transition-colors" title="Cancel">
                        ✕
                      </button>
                    </>
                  ) : (
                    <button
                      ...
                      className="p-1.5 text-[#838894] hover:text-white transition-colors" title="Rename section">
                      <PencilIcon className="w-4 h-4" />
                    </button>
                  )}
                  <button
                    onClick={() => handleDeleteSection(section._id)}
                    className="p-1.5 text-[#838894] hover:text-red-400 transition-colors" title="Delete section">
              ...
              {isOpen && (
                <div className="bg-[#0F1117] flex flex-col">
                  ...
                  {lectures.map((lecture) => (
                    <div key={lecture._id}
                      ...
                      <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0">
                        <button
                          onClick={() => openEditModal({ ...lecture, sectionId: section._id })}
                          className="p-1.5 text-[#838894] hover:text-white transition-colors" title="Edit lecture">
                          <PencilIcon className="w-3.5 h-3.5" />
                        </button>
                        <button
                          onClick={() => handleDeleteLecture(section._id, lecture._id)}
                          className="p-1.5 text-[#838894] hover:text-red-400 transition-colors" title="Delete lecture">
                  ...
                  <div className="flex items-center gap-2 px-6 py-2.5 border-t border-[#1C2333]">
                    ...
                    <button
                      onClick={() => {
                        // TODO: open material modal
                        toast("Material upload coming soon")
                      }}
