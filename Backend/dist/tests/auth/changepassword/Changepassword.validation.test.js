...
describe(`put ${URL} → VALIDATION`, () => {
    ...
    it("should fail if newPassword is less than 8 characters", async () => {
        ...
        expect(res.statusCode).toBe(400);
        expect(res.body.message).toBe("New Password length shouldn't be less than 8");
    });
    it("should fail if newPassword has no uppercase", async () => {
        ...
        expect(res.statusCode).toBe(400);
        expect(res.body.message).toBe("New Password should include atlist 1 uppercase character");
    });
    it("should fail if newPassword has no lowercase", async () => {
        ...
        const res = await request(app)
            ...
            .send({ ...changePasswordPayload, newPassword: "NEWPASSWORD@456" });
        expect(res.statusCode).toBe(400);
        expect(res.body.message).toBe("New Password should include atlist 1 lowercase character");
    });
    it("should fail if newPassword has no number", async () => {
        ...
        expect(res.statusCode).toBe(400);
        expect(res.body.message).toBe("New Password should include atlist 1 number character");
    });
    it("should fail if newPassword has no special character", async () => {
        ...
        expect(res.statusCode).toBe(400);
        expect(res.body.message).toBe("New Password should include atlist 1 special character");
    });
});
//# sourceMappingURL=Changepassword.validation.test.js.map