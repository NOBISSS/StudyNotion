...
export const createCategory = asyncHandler(async (req, res) => {
    ...
    ApiResponse.created(res, { category: categoryDetails }, "Category created successfully");
});
...
export const deleteCategory = asyncHandler(async (req, res) => {
    ...
    await Course.updateMany({ categoryId: categoryId }, { $set: { categoryId: generateCategory._id } });
...
export const getAllCategory = asyncHandler(async (req, res) => {
    const getCategory = await Category.find({ isActive: true }, { name: true, description: true });
    ApiResponse.success(res, {
...
export const categoryPageDetails = asyncHandler(async (req, res) => {
    ...
    const selectedCategory = await Category.findById(categoryId)
        ...
        path: "courses",
        match: {
            $or: [{ status: "Published" }, { isScheduled: true }],
            isActive: true,
        },
    })
        .exec();
    ...
    const categoriesExceptSelected = await Category.find({
        ...
        path: "courses",
        match: { $or: [{ status: "Published" }, { isScheduled: true }], isActive: true },
    });
