...
const WS_INTERNAL_URL = process.env.WS_INTERNAL_URL || "http://127.0.0.1:3002/announce";
const INTERNAL_SECRET = process.env.INTERNAL_SECRET;
async function notifyWebSocketServer(courseId, instructorId, announcement) {
    try {
...
export const makeAnnouncement = asyncHandler(async (req, res) => {
    ...
    if (!(await isValidInstructor(new Types.ObjectId(courseId), userId, req.accountType))) {
        throw AppError.forbidden("Only instructors can make announcements for their courses");
    }
    ...
    ApiResponse.success(res, { announcement }, "Announcement created successfully");
});
export const getAnnouncements = asyncHandler(async (req, res) => {
    ...
    if (req.path.includes("getread")) {
        announcementsWithReadStatus = announcementsWithReadStatus.filter((ann) => ann.isReaded);
    }
    else if (req.path.includes("getunread")) {
        announcementsWithReadStatus = announcementsWithReadStatus.filter((ann) => !ann.isReaded);
    }
    ApiResponse.success(res, { announcements: announcementsWithReadStatus }, "Announcements retrieved successfully");
});
export const markAnnouncementReadOrUnread = asyncHandler(async (req, res) => {
