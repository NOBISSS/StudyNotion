...
export const getWishlist = asyncHandler(async (req, res) => {
    ...
    const wishlist = await Wishlist.findOne({ userId }).populate({
        path: "courseIds",
        select: "courseName description originalPrice discountPrice thumbnailUrl typeOfCourse categoryId",
    });
    if (!wishlist) {
        ApiResponse.success(res, { wishlist: { userId, courseIds: [] } }, "Wishlist fetched successfully");
        return;
    }
    const wishlistWithCourseRatings = await Promise.all(wishlist.courseIds.map(async (course) => {
...
export const removeFromWishlist = asyncHandler(async (req, res) => {
    ...
    await wishlist.save();
    ApiResponse.success(res, { wishlist }, "Course removed from wishlist successfully");
});
