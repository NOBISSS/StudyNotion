...
export const createOrder: Handler = asyncHandler(async (req, res) => {
  ...
  if (course.typeOfCourse === "Free")
    throw AppError.badRequest(
      "This course is free — use enroll endpoint instead",
    );

  // ── Block if already enrolled ─────────────────────────────────────────────
  const alreadyEnrolled = await CourseEnrollment.exists({
    courseId,
    userId: studentId,
  });
  if (alreadyEnrolled)
...
export const verifyPayment: Handler = asyncHandler(async (req, res) => {
  ...
  const enrollment = await CourseEnrollment.create({
    ...
    amountPaid: payment.finalAmount / 100, // convert back to rupees
    enrolledAt: new Date(),
  });
  ...
  ApiResponse.success(
    res,
    { enrolled: true, enrollmentId: enrollment._id },
